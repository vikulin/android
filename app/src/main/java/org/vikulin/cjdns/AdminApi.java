...
public class AdminApi {
            ...
            return Observable.create(new BaseOnSubscribe<Map<String, Long>>(api, new Request("FileNo_import",
                ...
                protected Map<String, Long> parseResult(final Map response) {
                    return new HashMap<String, Long>() {{
                        Log.i(TAG,"response:"+(Long) response.get(wrapString("tunfd")));
    ...
    private static Map sendAuthenticatedRequest(Request request, AdminApi api) throws NoSuchAlgorithmException, IOException {
        ...
        Log.i(TAG, "request:"+new String(requestBytes));
        if(request.name.equals("Core_initTunfd")){
            Map<ByteBuffer, ByteBuffer> r = parse(requestBytes);
            Log.i(TAG, "parsed request:");
            for(Map.Entry<ByteBuffer, ByteBuffer> entry : r.entrySet()){
                //System.out.println(new String(entry.getKey().array()) + ":" + new String(entry.getValue().array()));
            }
        }
        // Send request.
    ...
    private static String getCookie(AdminApi api) throws IOException {
        Log.i("sent getCookie "+TAG, "request:"+REQUEST_COOKIE);
    ...
    private static Map send(Map request, AdminApi api) throws IOException {
        ...
        try {
            ...
            Log.e(TAG, "Socket timeout:"+SOCKET_TIMEOUT+" msec");
            Log.i("sent "+TAG, "request:"+new String(data));
            throw e;
            //send(request, api);
        }
        socket.close();
